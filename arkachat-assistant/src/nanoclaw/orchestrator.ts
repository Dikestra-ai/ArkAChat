...
import { emitEvent } from '../orchestration/coordinator.js';
...
export async function route(args: {
  ...
  try {
    // ── Owners and employees: scripts-first, LLM fallback. ─────────────
    if (args.user.role === 'owner' || args.user.role === 'employee') {
      ...
      if (intent && intent.confidence >= 0.5) {
        ...
        if (script) {
          ...
          // Broadcast routing event to Gibraltar-Code (non-blocking)
          emitEvent('assistant.routed', {
            route: 'script',
            handler: script.name,
            channel: args.channel,
            userId: args.user.id,
      ...
      emitEvent('assistant.routed', { route: 'llm_owner', channel: args.channel, userId: args.user.id }).catch(() => {});
      return { reply, route: 'llm_owner', latencyMs };
    }

    // ── Clients: RAG-first, then draft-for-owner. ────────────────────────
    ...
    if (rag.grounded) {
      ...
      emitEvent('assistant.routed', { route: 'rag_grounded', channel: args.channel, userId: args.user.id }).catch(() => {});
    ...
    // Notify web/proxy services that a draft is awaiting owner approval
    emitEvent('assistant.draft_pending', {
      draftId: draft.id,
      businessId: args.user.business_id,
      clientUserId: args.user.id,
      channel: args.channel,
